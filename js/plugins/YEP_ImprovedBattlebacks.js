...
Sprite_ImprovedBattleback.prototype.updateScroll = function() {
  ...
  if (this._scrollSpeedX != 0) this._xTimer -= 1;
  if (this._xTimer <= 0) {
    this.origin.x += this._scrollSpeedX > 0 ? shift : (shift*-1);
    this._xTimer = Math.abs(this._scrollSpeedX);
  }

  if (this._scrollSpeedY != 0) this._yTimer -= 1;
  if (this._yTimer <= 0) {
    this.origin.y += this._scrollSpeedY > 0 ? shift : (shift*-1);
    this._yTimer = Math.abs(this._scrollSpeedY);
  }
};
