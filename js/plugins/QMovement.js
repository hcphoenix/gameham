  ...
  Game_Player.prototype.moveByInput = function() {
    if (!this.startedMoving() && this.canMove()) {
      ...
      if (Imported.QInput && Input.preferGamepad() && $gameMap.offGrid()) {
        ...
        // FORCE DOWNWARD MOVE
        // left down
        if ([7, 4, 1].contains(direction)) {
            this.moveInputDiagonal(1);
        }
        // right down
        else if ([9, 6, 3].contains(direction)) {
            this.moveInputDiagonal(3);
        } else {
            this.moveInputVertical(2);
        }
        /* Old movement code: dumb, bad
