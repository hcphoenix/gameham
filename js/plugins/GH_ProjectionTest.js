...
Scene_Nasu.prototype.createDisplayObjects = function() {
    ...
    this.children.forEach(x => x.opacity = 0);
    
    this._camera = new PIXI.projection.Camera3d(); 
    this._camera.setPlanes(300, 1, 10090, false);
    this._camera.position.set(Graphics.boxWidth / 2, 0);
    this._camera.position3d.y = -500;
    this.addChild(this._camera);

    const groundLayer = new PIXI.projection.Container3d();
    groundLayer.euler.x = Math.PI / 2;
    //groundLayer.affine = PIXI.projection.AFFINE.AXIS_X;
    this._camera.addChild(groundLayer);

    const fg = new PIXI.projection.Sprite3d();
    this._fg = fg;
    //fg.bitmap= this._intro_img;
    groundLayer.addChild(fg);

    fg.anchor.set(0.5, 0.5);
    console.log(fg);
    console.log(this._intro_img);
    fg.texture = PIXI.Texture.from(this._intro_img._url);
}
...
Scene_Nasu.prototype.update_start_phase = function() {
    this._fg.euler.z += 0.03;

    if (Input.isPressed("left")) {
        this._camera.position3d.x += 3;
    }
    if (Input.isPressed("right")) {
        this._camera.position3d.x -= 3;
    }
    if (Input.isPressed("up")) {
        this._camera.position3d.y += 3;
    }
